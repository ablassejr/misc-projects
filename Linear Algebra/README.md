...
Every algorithm is implemented from scratch in C against raw `double*` arrays. C++ wraps it with RAII and tests. Python verifies against NumPy. TypeScript visualizes via WebAssembly. **No library ever does the computation** -- libraries only verify your output.
...
Open `engine/src/la_<chapter>.c` and fill in the function body. Raw arrays, explicit index computation via `LA_IDX(i, j, cols)`. No abstractions. Every `data[i * cols + j]` is a decision you make about how matrices are stored.
...
The Catch2 tests are already written. They call your C functions through the C++ `Matrix` class. Rebuild and run:
...
The verification scripts call your C shared library via `ctypes` and compare every result against NumPy:
...
|---------|------|-----------|------------------------|
| 1. Systems | `la_elimination.c` | `swap_rows`, `scale_row`, `add_scaled_row`, `find_pivot`, `to_ref`, `to_rref`, `solve` | Plug solution back into original system: residual < epsilon |
| 2. Matrices | `la_ops.c` | `add`, `scalar_mul`, `mul`, `mul_ikj`, `transpose`, `inverse`, `lu_factorize`, `forward_sub`, `back_sub`, `elementary_*` | `A * inv(A) = I`, `L * U = A`, `(AB)^T = B^T A^T` |
| 3. Determinants | `la_determinant.c` | `minor`, `cofactor`, `det_cofactor`, `det_elimination`, `cofactor_matrix`, `adjoint`, `inverse_adjoint`, `cramers_rule` | `det(AB) = det(A) * det(B)`, `det(A^T) = det(A)`, `det(inv(A)) = 1/det(A)` |
| 4. Vector Spaces | `la_vecspace.c` | `rank`, `nullity`, `null_space`, `column_space`, `row_space`, `is_independent`, `is_in_span`, `change_of_basis` | `rank(A) + nullity(A) = n`, null space vectors satisfy `Ax = 0` |
| 5. Inner Products | `la_inner.c` | `dot`, `norm`, `normalize`, `distance`, `angle`, `are_orthogonal`, `cross_product`, `gram_schmidt`, `orthogonal_projection`, `least_squares` | GS output: all pairs orthogonal + unit norm, LS residual orthogonal to column space |
...
Never use `== 0` to check a pivot. Always use `fabs(value) < LA_EPSILON`.

## Prerequisites

- C compiler (Apple Clang or GCC)
- CMake 3.20+
