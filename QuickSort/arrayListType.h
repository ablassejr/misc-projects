...
public:
  ...
  int partition(int first, int middle, int last);
  // Function to partition the array for quicksort

  void swap(int first, int second);
  // Function to swap two elements of the array

  void recQuickSort(int first, int middle, int last);
...
template <class elemType>
int arrayListType<elemType>::partition(int first, int middle, int last) {
  elemType pivot, pivotArray[3];
  int index, smallIndex;
  int indexArray[3] = {first, middle, last};
  for (int i = 0; i < 3; i++) {
    retrieveAt(indexArray[i], pivotArray[i]);
  }
  cout << "Current List: ";   
  for (int i = 0; i < length; i++) {
    cout << list[i] << " ";
  ...
  cout << std::endl; 
  // pivot is the median of the first, middle, last elements
  std::sort(pivotArray, pivotArray + 3);
  pivot = pivotArray[1];
  cout << "Pivot: " << pivot << endl;

  // Find which index contains the pivot value and swap to first position
  for (int i = 0; i < 3; i++) {
    if (list[indexArray[i]] == pivot) {
      swap(first, indexArray[i]);
      break;
    }
  }
  ...
  for (index = first + 1; index <= last; index++) {
    if (list[index] < pivot) {
      ...
      cout << "Swapped " << list[smallIndex] << " and " << list[index]
           << endl << "Partition is at position " << smallIndex << endl;
    }
  }
...
template <class elemType>
void arrayListType<elemType>::recQuickSort(int first, int middle, int last) {
  int pivotIndex;

  if (first < last) {
    pivotIndex = partition(first, middle, last);

    recQuickSort(first, (first + pivotIndex - 1) / 2, pivotIndex - 1);
    recQuickSort(pivotIndex + 1, (pivotIndex + 1 + last) / 2, last);
  }
}

template <class elemType> void arrayListType<elemType>::quickSort() {
  recQuickSort(0, (length - 1) / 2, length - 1);
}

enum class pivots { first, middle, last };